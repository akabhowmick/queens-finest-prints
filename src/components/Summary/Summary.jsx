...
class Summary extends React.Component {
  ...
  setCurrentTotal = () => {
    let currentTotal = 0;
    const cartItems = this.props.data.cartItems;

    if (cartItems.length > 0) {
      for (let i = 0; i < cartItems.length; i++) {
        currentTotal += cartItems[i].price * cartItems[i].quantity;
      }
    }

    // Use the callback version of setState to ensure that setFinalCost executes with updated state
    this.setState({ cartSubTotal: currentTotal }, () => {
      this.setFinalCost();
    });
  };
  ...
  setFinalCost = () => {
    let currentFinal =
      this.state.cartSubTotal + this.props.data.shippingCost - this.props.data.finalDiscount;
  ...
  handleInputClick = (e) => {
    ...
    if (this.state.codeEntered === "20OFF") {
      ...
      discountAmount = Math.round(100 * (this.state.cartSubTotal * 0.1)) / 100;
    }
      ...
      if (this.props.data.cartItems.length === 0) {
        btn = (
          <button
            id="disabled"
            type="button"
            onClick={(e) => this.handleShippingBtn(e)}
            className="btn-primary round-pill"
          >
            Shipping Info
          </button>
        );
      ...
      if (this.props.errors === true) {
        btn = (
          <button
            id="disabled"
            type="button"
            onClick={(e) => this.handlePaymentBtn(e)}
            className="btn-primary round-pill"
          >
            Validate Address to go to Payment!
          </button>
        );
      } else {
        btn = (
          ...
          >
            ...
            {(
              ...
              this.props.data.shippingCost +
              this.state.taxAmount -
              this.props.data.finalDiscount
            ).toFixed(2)}
          </button>
        );
      }
      ...
      if (this.props.paymentSuccess === false) {
        btn = (
          <button
            id="disabled"
            type="button"
            onClick={(e) => this.handleConfirmBtn(e)}
            className="btn-primary round-pill"
          >
          ...
          <div className="cart-summary-div">
            <div className="cart-summary">
              ...
              <input
                ...
                name="promo"
              />
              <button className="btn-primary round-pill" onClick={(e) => this.handleInputClick(e)}>
                Apply
              </button>
            </div>
            <div className="cart-info">
            ...
            <button
              type="button"
              onClick={(e) => this.handleCancelBtn(e)}
              className="cancelbtn btn-primary round-pill"
            >
              Back to Store
            </button>
            {btn}
